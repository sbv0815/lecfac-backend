...
async def eliminar_facturas_multiples(factura_ids: List[int]):
...
@app.get("/admin/productos/catalogo")
...
@app.get("/admin/duplicados/facturas")
async def detectar_facturas_duplicadas():
    ...
    try:
        ...
        # Detectar duplicados por establecimiento + total similar
        duplicados = []
...
@app.get("/admin/duplicados/productos")
...
@app.post("/admin/productos/fusionar")
async def fusionar_productos(producto_mantener_id: int, producto_eliminar_id: int):
    """Fusionar dos productos duplicados"""
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        
        # Actualizar todos los precios del producto a eliminar
        cursor.execute("""
...
@app.put("/admin/productos/{producto_id}/editar")
async def editar_producto_catalogo(producto_id: int, nombre: str, codigo_ean: Optional[str] = None):
