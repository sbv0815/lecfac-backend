...
async def obtener_listas_usuario(usuario_id: int, estado: str = "activa"):
    ...
    try:
        ...
        return {"success": True, "listas": listas, "total": len(listas)}
    ...
    try:
            ...
            items.append(
                {
                    ...
                    "precio_formateado": (
                        f"${row[7]:,.0f}".replace(",", ".") if row[7] else "$0"
                    ),
        ...
        return {
            ...
            "lista": {
                ...
                "total_formateado": (
                    f"${lista_row[5]:,.0f}".replace(",", ".") if lista_row[5] else "$0"
                ),
            ...
            "resumen": {
                ...
                "progreso": (
                    round(comprados / total_items * 100, 0) if total_items > 0 else 0
                ),
            },
        }
...
async def marcar_item_comprado(item_id: int, data: MarcarCompradoRequest):
    ...
    try:
        ...
        return {
            "success": True,
            "mensaje": (
                "Item marcado como comprado" if data.comprado else "Item desmarcado"
            ),
        }
...
async def eliminar_lista(lista_id: int, usuario_id: int):
    ...
    try:
        ...
        if database_type == "postgresql":
            cursor.execute(
                "SELECT usuario_id FROM listas_compras WHERE id = %s", (lista_id,)
            )
        else:
            cursor.execute(
                "SELECT usuario_id FROM listas_compras WHERE id = ?", (lista_id,)
            )
        ...
        return {"success": True, "mensaje": "Lista archivada correctamente"}
