...
Procesa facturas de manera asíncrona y completamente automatizada
"""
...
from database import get_db_connection, detectar_cadena
...
class OCRProcessor:
        ...
        try:
            ...
            if result.get("success"):
                self._process_successful_ocr(
                    cursor, factura_id, result["data"], user_id
                )
                conn.commit()
    ...
    def _process_successful_ocr(self, cursor, factura_id: int, data: Dict, user_id: int):
        ...
        for idx, prod in enumerate(productos):
            if self.validate_product(prod):
                producto_id = self._save_product(cursor, prod, factura_id, establecimiento, cadena)
                if producto_id:
    ...
    def _save_product(self, cursor, product: Dict, factura_id: int, 
                     establecimiento: str, cadena: str) -> Optional[int]:
        """Guarda un producto y retorna su ID"""
        try:
            codigo = str(product.get("codigo", "SIN_CODIGO")).strip()
            nombre = str(product.get("nombre", "Producto sin nombre")).strip()
            precio = float(product.get("precio", 0))
            cantidad = int(product.get("cantidad", 1))
            
            # Manejar códigos especiales
            if codigo == "SIN_CODIGO" or len(codigo) < 3:
                # Generar código único basado en nombre
                import hashlib
                hash_nombre = hashlib.md5(nombre.encode()).hexdigest()[:8].upper()
                codigo = f"AUTO_{hash_nombre}"
            ...
            if resultado:
                ...
                es_fresco = len(codigo) < 6 or codigo.startswith("AUTO_") or codigo.startswith("PLU_")
                ...
                cursor.execute("""
                    ...
                    VALUES (%s, %s, %s, %s, 1, %s) 
                    RETURNING id
                """, (codigo, nombre, es_fresco, datetime.now(), datetime.now()))
                
                producto_id = cursor.fetchone()[0]
            
            # Guardar precio en histórico
            for _ in range(cantidad):  # Si hay múltiples unidades, crear múltiples registros
                cursor.execute("""
...
def detectar_cadena(establecimiento: str) -> str:
    ...
    Esta es una función auxiliar si no está en database.py
    """
    ...
    cadenas = {
        'exito': ['exito', 'éxito', 'almacenes exito'],
        'carulla': ['carulla', 'carulla fresh'],
        'jumbo': ['jumbo'],
