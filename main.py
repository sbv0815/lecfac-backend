...
async def crear_producto_papa(producto_id: int, request: Request):
    ...
    ✅ CORREGIDO: Usa estado='verificado' en lugar de 'validado'
    ...
    try:
        ...
        # ✅ Generar codigo_lecfac desde el nombre
        ...
        # 1️⃣ Actualizar el PAPA con los datos validados + codigo_lecfac
        cursor.execute(
            """
        ...
        papa_codigo_lecfac = resultado[2]

        print(f"   ✅ PAPA actualizado: {papa_nombre}")
        print(f"   📛 Código LecFac: {papa_codigo_lecfac}")

        # 2️⃣ Obtener PLUs del PAPA en productos_por_establecimiento
        cursor.execute(
            """
        ...
        # 3️⃣ Buscar y actualizar HIJOS (mismo PLU + mismo establecimiento)
        ...
        return {
            ...
            "codigo_lecfac": papa_codigo_lecfac,
...
async def comparar_precios_producto(codigo_lecfac: str):
    """
    Retorna el ÚLTIMO PRECIO de un producto en cada establecimiento
    Solo usa datos del producto PAPA (precios actualizados)
    """
    conn = get_db_connection()
    cursor = conn.cursor()

    try:
        # 1. Obtener el producto PAPA
        cursor.execute(
            """
            SELECT id, nombre_consolidado, marca, codigo_ean
            FROM productos_maestros_v2
            WHERE codigo_lecfac = %s AND es_producto_papa = TRUE
        """,
            (codigo_lecfac,),
        )
        ...
        papa_id, nombre, marca, ean = papa

        # 2. Obtener precios del PAPA por establecimiento
        cursor.execute(
            """
            SELECT
            ...
            precios.append(
                {
                    ...
                    "vigente": vigente,
                    "es_mejor_precio": precio == precio_min,
                    "diferencia_vs_mejor": int(diferencia),
        ...
        return {
            ...
            "precios": precios,
            "mejor_precio": {
                "establecimiento": precios[0]["establecimiento"],
